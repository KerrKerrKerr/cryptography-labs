...
pub fn draw_gamming<'a>(input: &'a str, seed: &'a str, output: &'a str, decrypted: &'a str) -> Element<'a, Message> {
    responsive(move |size| {
        let gamming_input = text_input("Type plain text", input)
            .on_input(Message::GammingInput)
            .padding(10)
            .size(20)
            .width(Length::Fill);

        let gamming_seed = text_input("Seed (number, e.g. 12345)", seed)
        ...
        let right_block = column![
            Text::new("Encrypted (hex)"),
            text_input("Ciphertext hex", output)
        ...
        column![
            ...
            Text::new("Stream cipher: plaintext is XOR-ed with a pseudo-random gamma generated by Xorshift32 PRNG. Enter text and seed to encrypt, then press Decrypt.").size(16),
            io_layout,
            decrypted_area,
        ]
        .spacing(12)
...
pub fn draw_frequency_analysis<'a>(
    ...
    substitution: &'a FrequencySubstitution,
) -> Element<'a, Message> {
    responsive(move |_size| {
        ...
        if !result.is_empty() {
            ...
            for (ch, count, percentage) in result {
                ...
                // Current replacement character from substitution map
                let replacement = substitution.map.get(ch).map(|c| c.to_string()).unwrap_or_default();
                let replacement_input = text_input("Replace with", &replacement)
                    .on_input(move |new_val| {
                        let new_char = new_val.chars().next().unwrap_or(*ch);
                        Message::FreqAnalysisReplace(*ch, new_char)
                    })
                ...
                let row_data = row![
                    container(Text::new(format!("'{}'", ch)).size(14)).width(Length::Fixed(60.0)),
                    container(Text::new(count.to_string()).size(14)).width(Length::Fixed(50.0)),
                    container(Text::new(format!("{:.2}%", percentage)).size(14)).width(Length::Fixed(80.0)),
                    container(replacement_input).width(Length::Fixed(100.0)),
